<footer>
    <div class="footer-contenido-top">
        <div class="footer-bloque-contacto">
        </div>
    </div>

    <div class="footer-grid-principal">
        <div class="footer-col">
            <h4>¡Escribenos!</h4>
            <p>Envianos tu consulta y te contactaremos.</p>
            <br><a href="<?= base_url('/contacto') ?>"> Contacto</a>
        </div>

        <div class="footer-col">
            <h4>Horarios</h4>
            <ul class="footer-lista">
                <li>Lunes a Viernes</li>
                <li>8hs a 20hs</li>
            </ul>
        </div>

        <div class="footer-col">
            <h4>Localidad</h4>
            <ul class="footer-lista">
                <li>San Luis</li>
                <li>La Punta</li>
            </ul>
        </div>

        <div class="footer-col">            
            <h4 class="mt-3">Nuestras Redes</h4>
            <div class="footer-redes">
                <div class="red"><i class="fa-brands fa-whatsapp"></i><a href="#" target="_blank">+54 9 266 470-3362</a> - Solo WhatsApp</div>
                <div class="red"><i class="fa-brands fa-instagram"></i><a href="https://www.instagram.com/puntoarsanluis?igsh=M3V4YndhNjBoamRh" target="_blank">PuntoAR</a></div>
                <div class="red"><i class="fa-solid fa-envelope"></i><a href="" target="_blank">Escribanos por correo</a></div>
            </div>
        </div>
    </div>

    <div class="footer-pie">
        &copy; PuntoAR. Todos los derechos reservados.
    </div>
</footer>
</body>

</html>